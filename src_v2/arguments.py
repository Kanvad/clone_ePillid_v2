...
def strtobool(value: str) -> bool:
    ...
    n, no, f, false, off and 0. Raises ValueError if value is anything else.
    """
    ...
    if value in ("y", "yes", "t", "true", "on", "1"):
        ...
        raise ValueError(f"truthy value expected: got {value!r}")
...
def common_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default="1",
        type=str,
        help="switches to enhanced augmentation with perspective",
    )
    ...
    parser.add_argument(
        "--pooling",
        default="GAvP",
        choices=["MPNCOV", "CBP", "BCNN", "GAvP"],
        help="pooling layer for the appearance embeddings",
    )

    parser.add_argument(
        "--metric_margin", type=float, default=1.0, help="margin for the contrastive loss in training"
    )
    parser.add_argument(
        "--metric_embedding_dim",
        type=int,
        default=2048,
        help="dimensionality of the embedding feature vector used for the contrastive loss learning",
    )
    parser.add_argument(
        "--train_with_side_labels",
        default="1",
        type=str,
        help="train with side info i.e. front and back will be treated as different classes",
    )
    parser.add_argument(
        "--metric_simul_sidepairs_eval",
        default="1",
        type=str,
        help="evals holdout and val simulating per pill side img pairs",
    )
    parser.add_argument(
        "--sidepairs_agg",
        type=str,
        ...
        help="aggregation method for embeddings. post_*: agg. after calculating the distance",
    )
    parser.add_argument(
        "--metric_evaluator_type",
        type=str,
        default="cosine",
        choices=["euclidean", "cosine", "ann"],
...
def nocv_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--label_encoder", default="folds/pilltypeid_nih_sidelbls0.01_metric_5folds/base/label_encoder_pytorch131.pickle"
    )

    return parser


def cv_parser() -> argparse.ArgumentParser:
    parser = common_parser()
...
def parse_bool_args(args: argparse.Namespace) -> argparse.Namespace:
    """Parse string boolean arguments to actual booleans."""
    bool_fields = [
        "add_persp_aug",
        "train_with_side_labels",
        "metric_simul_sidepairs_eval",
    ...
    for field in bool_fields:
        if hasattr(args, field):
            ...
            if isinstance(value, str):
                setattr(args, field, strtobool(value))
    return args
